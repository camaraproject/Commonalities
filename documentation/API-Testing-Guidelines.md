...
- [Test Cases contributions for APIs](#contribution)
...
A first step is to identify potential environment variables writing the test plan, e.g. device identifiers (phone numbers, IP addresses), status of a testing device, etc. 

This list of input data to be requested may be included as information in the feature file, as a comment block just after the `Feature` header. A name may be assigned to ease reference in the scenarios, e.g.:
...
Testing plans should identify the list of input values that have to be provided by the implementation.
