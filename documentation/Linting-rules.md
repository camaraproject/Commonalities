...
- [1. Spectral core ruleset](#1-spectral-core-ruleset)
- [2. Language](#2-language)
- [3. API Definition](#3-api-definition)
- [4. Summary of proposed CAMARA rules](#4-summary-of-proposed-camara-rules)
- [5. OWASP API Security Top 10 2023 rules](#5-owasp-api-security-top-10-2023-rules)
...
## 5. OWASP API Security Top 10 2023 rules
Spectral rules from [Stoplight API Stylebook](https://apistylebook.stoplight.io/docs/owasp-top-10-2023) implement checks for [OWASP API Security Top 10 2023](https://owasp.org/API-Security/editions/2023/en/0x11-t10/). [These rules](https://github.com/stoplightio/spectral-owasp-ruleset) can be used with Spectral to automatically lint OpenAPI documents for security issues.

Columns **Use in CAMARA** and **CAMARA severity** include modifications related to original OWASP ruleset - **Target CAMARA severity** column includes modification for 2027 releases. 

| **Rule Name**                                     | **Severity**              | **Description**                                                                                              | **Comment**                                                                                                                                                                    | **Use in CAMARA** | **CAMARA severity**       | **Target CAMARA severity** |
|---------------------------------------------------|---------------------------|--------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------|---------------------------|----------------------------|
| owasp:api1:2023-no-numeric-ids                    | :exclamation: error       | Use random IDs that cannot be guessed. UUIDs are preferred but any other random string will do.              | Prevents broken object level authorization by ensuring ID parameters are not numeric/enumerable integers. Numeric IDs make it easy for attackers to iterate through resources. | Yes               | :exclamation: error       | :exclamation: error        |
| owasp:api2:2023-auth-insecure-schemes             | :exclamation: error       | Authentication scheme is considered outdated or insecure.                                                    | Some HTTP authorization schemes are now considered insecure, such as NTLM or OAuth v1. Use modern alternatives.                                                                | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api2:2023-jwt-best-practices                | :exclamation: error       | Security schemes using JWTs must explicitly declare support for RFC8725 in the description.                  | RFC8725 describes common JWT pitfalls like ignoring algorithms or using insecure algorithms. Implementation should conform to these best practices.                            | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api2:2023-no-api-keys-in-url                | :exclamation: error       | API Key passed in URL.                                                                                       | API Keys passed in headers, cookies or query parameters can be eavesdropped, especially in URLs as logging/history tools will track them.                                      | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api2:2023-no-credentials-in-url             | :exclamation: error       | Security credentials detected in path parameter.                                                             | URL parameters MUST NOT contain credentials such as API key, password, or secret as they appear in logs and browser history.                                                   | Yes               | :exclamation: error       | :exclamation: error        |
| owasp:api2:2023-no-http-basic                     | :exclamation: error       | Security scheme uses HTTP Basic. Use a more secure authentication method, like OAuth 2, or OpenID.           | Basic authentication credentials transported over network are more susceptible to interception. Passwords and tokens are not encrypted.                                        | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api2:2023-short-lived-access-tokens         | :exclamation: error       | Authentication scheme does not appear to support refresh tokens, meaning access tokens likely do not expire. | Using short-lived access tokens with refresh tokens is a good practice. Reduces the window for malicious use of compromised tokens.                                            | Yes               | :exclamation: error       | :exclamation: error        |
| owasp:api2:2023-write-restricted                  | :exclamation: error       | This write operation is not protected by any security scheme.                                                | All write operations (POST, PUT, PATCH, DELETE) must be secured by at least one security scheme to prevent unauthorized modifications.                                         | Yes               | :exclamation: error       | :exclamation: error        |
| owasp:api4:2023-array-limit                       | :exclamation: error       | Schema of type array must specify `maxItems`.                                                                | Array size should be limited to mitigate resource exhaustion attacks. Prevents unbounded arrays from consuming excessive resources.                                            | Yes               | :warning: **warn**        | :exclamation: error        |
| owasp:api4:2023-integer-limit                     | :exclamation: error       | Schema of type integer must specify minimum and maximum.                                                     | Integers should be limited to avoid negative numbers when positive expected, or reduce unreasonable iterations (e.g., 1000 times vs 10).                                       | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api4:2023-integer-limit-legacy              | :exclamation: error       | Schema of type integer must specify minimum and maximum.                                                     | Same as integer-limit but for OAS 2.0 and OAS **3.0.x** which handle number constraints differently than OAS 3.1.x.                                                            | Yes               | :warning: **warn**        | :exclamation: error        |
| owasp:api4:2023-integer-format                    | :exclamation: error       | Schema of type integer must specify format (int32 or int64).                                                 | Specifying integer format helps prevent resource exhaustion by making size constraints explicit.                                                                               | Yes               | :warning: **warn**        | :exclamation: error        |
| owasp:api4:2023-rate-limit                        | :exclamation: error       | All 2XX and 4XX responses should define rate limiting headers.                                               | Proper rate limiting prevents attackers from overloading the API. Use IETF RateLimit headers or custom headers like X-RateLimit-Limit.                                         | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api4:2023-rate-limit-retry-after            | :exclamation: error       | A 429 response should define a Retry-After header.                                                           | Setting Retry-After helps well-meaning consumers back off gracefully without exacerbating problems.                                                                            | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api4:2023-string-limit                      | :exclamation: error       | Schema of type string must specify `maxLength`, `enum`, or `const`.                                          | String size should be limited to mitigate resource exhaustion attacks. Use `maxLength`, `enum`, or `const` to constrain values.                                                | Yes               | :warning: **warn**        | :exclamation: error        |
| owasp:api5:2023-admin-security-unique             | :exclamation: error       | Admin endpoint has the same security requirement as a non-admin endpoint.                                    | Administrative functions should use different security schemes than regular functions to prevent privilege escalation attacks.                                                 | Yes               | :exclamation: error       | :exclamation: error        |
| owasp:api8:2023-define-cors-origin                | :exclamation: error       | CORS header should be defined on all responses.                                                              | Setting up CORS headers controls which websites can make browser-based HTTP requests to your API. Use wildcard, null, or specific origins.                                     | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api8:2023-no-scheme-http                    | :exclamation: error       | Server schemes must not use http. Use https or wss instead.                                                  | HTTP is inherently insecure and can lead to PII leakage through traffic sniffing or man-in-the-middle attacks. Use HTTPS/WSS.                                                  | Yes               | :exclamation: error       | :exclamation: error        |
| owasp:api8:2023-no-server-http                    | :exclamation: error       | Server URLs must not use `http://`. Use `https://` or `wss://` instead.                                      | Server URLs must use secure protocols. HTTP can expose sensitive information through unencrypted traffic.                                                                      | Yes               | :exclamation: error       | :exclamation: error        |
| owasp:api9:2023-inventory-access                  | :exclamation: error       | Declare intended audience of every server by defining `servers[0].x-internal` as true/false.                 | Use `x-internal` vendor extension (true/false) to explicitly declare if a server is internal or public-facing for documentation.                                               | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api9:2023-inventory-environment             | :exclamation: error       | Declare intended environment in server descriptions using terms like local, staging, production.             | Make it clear which servers run in which environment to avoid exposing test data publicly or bypassing security measures.                                                      | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api2:2023-read-restricted                   | :warning: warn            | This read operation is not protected by any security scheme.                                                 | Read operations (GET, HEAD) should be secured by at least one security scheme to prevent unauthorized data access.                                                             | Yes               | :warning: warn            | :warning: warn             |
| owasp:api3:2023-constrained-additionalProperties  | :warning: warn            | Objects should not allow unconstrained additionalProperties.                                                 | When `additionalProperties` are not disabled, constrain them with `maxProperties` to prevent mass assignment vulnerabilities.                                                  | Yes               | :warning: warn            | :warning: warn             |
| owasp:api3:2023-constrained-unevaluatedProperties | :warning: warn            | Objects should not allow unconstrained `unevaluatedProperties`.                                              | When `unevaluatedProperties` are not disabled, constrain them with `maxProperties` to prevent mass assignment.                                                                 | Yes               | :warning: warn            | :warning: warn             |
| owasp:api3:2023-no-additionalProperties           | :warning: warn            | If the additionalProperties keyword is used it must be set to false.                                         | By default JSON Schema allows additional properties, potentially leading to mass assignment issues. Disable with `additionalProperties: false` or add `maxProperties`.         | Yes               | :warning: warn            | :warning: warn             |
| owasp:api3:2023-no-unevaluatedProperties          | :warning: warn            | If the `unevaluatedProperties` keyword is used it must be set to false.                                      | Similar to `additionalProperties` but for JSON Schema 2020-12 (OAS 3.1). Prevents unvalidated fields from being accepted.                                                      | Yes               | :warning: warn            | :warning: warn             |
| owasp:api4:2023-rate-limit-responses-429          | :warning: warn            | Operation is missing rate limiting response (code 429)}.                                                     | Defining a 429 response ensures rate limiting is properly implemented and documented, preventing resource exhaustion attacks.                                                  | **No**            | :warning: warn            | :warning: warn             |
| owasp:api4:2023-string-restricted                 | :warning: warn            | Schema of type string should specify a `format`, `pattern`, `enum`, or `const`.                              | To avoid unexpected values being sent or leaked, strings should have a `format`, RegEx `pattern`, `enum`, or `const` defined.                                                  | Yes               | :warning: warn            | :warning: warn             |
| owasp:api8:2023-define-error-validation           | :warning: warn            | Missing error response of either 400, 422 or 4XX.                                                            | Carefully define schemas for all API responses, including error responses for invalid requests to avoid leaking implementation details.                                        | Yes               | :warning: warn            | :warning: warn             |
| owasp:api8:2023-define-error-responses-401        | :warning: warn            | Operation is missing 401 response.                                                                           | Define 401 response schemas to ensure proper handling of unauthorized requests without leaking implementation details in backtraces.                                           | Yes               | :exclamation: **error**   | :exclamation: error        |
| owasp:api8:2023-define-error-responses-500        | :warning: warn            | Operation is missing 500 response.                                                                           | Define 500 response schemas to ensure internal server errors return proper JSON structure instead of exposing backtraces.                                                      | **No**            | :grey_exclamation: error  | :grey_exclamation: error   |
| owasp:api7:2023-concerning-url-parameter          | :information_source: info | Make sure to review the way this URL is handled to protect against Server Side Request Forgery.              | Parameters containing URLs (callback, redirect, *_url) can lead to SSRF if not properly validated. Review handling carefully.                                                  | Yes               | :information_source: info | :information_source: info  |
